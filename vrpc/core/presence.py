...
def _menu(state: GameState, settings, content: Content, language: str) -> dict:
    if state.card_id:
        large_image = content.card_square(state.card_id) or FALLBACK_LARGE_IMAGE
        large_text = f"{state.name}#{state.tag}" if state.name else t(language, "app_title")
    else:
        large_image, large_text = _large_card(state, content, language)

    details = t(language, "custom") if state.is_custom else (content.mode_name(state.queue_id) if state.queue_id else t(language, "p_in_menu"))
...
def _ingame(state: GameState, settings, content: Content, language: str) -> dict:
    ...
    details = t(language, "custom") if state.is_custom else content.mode_name(state.queue_id)
