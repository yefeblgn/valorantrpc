"""Sistem tepsisi (notify icon) — pystray ile."""
...
class Tray:
    def __init__(self, settings, callbacks: dict) -> None:
        ...
        self.cb = callbacks  # show, toggle_rpc, set_language, toggle_autostart, quit
        self._active = False
    ...
    def _build_menu(self) -> Menu:
        return Menu(
            ...
            Menu.SEPARATOR,
            MenuItem(
                self._txt("rpc_enabled"),
                lambda: self.cb["toggle_rpc"](),
            ...
            MenuItem(
                ...
                Menu(
                    MenuItem(
                        "Türkçe",
                        lambda: self.cb["set_language"]("tr"),
                        checked=lambda i: self.settings.language == "tr",
                        radio=True,
                    ),
                    MenuItem(
                        "English",
                        lambda: self.cb["set_language"]("en"),
                        checked=lambda i: self.settings.language == "en",
                        radio=True,
                    ),
                ),
            ),
            MenuItem(
                self._txt("autostart"),
                lambda: self.cb["toggle_autostart"](),
    ...
    # ---- güncelleme (poller thread'inden güvenli) ----
    def update(self, active: bool, status_text: str) -> None:
        try:
            ...
            logger.debug("Tray güncellenemedi: %s", e)
